...
if(isset($_POST['submit'])){
	...
	$startDate = date('Y-m-d', strtotime('-7', strtotime($date)));
	$endDate = $date;
}
...
<div>
  ...
  <p>Tabel Data Transaksi</p>
</div>
<ul class="app-breadcrumb breadcrumb side">
  <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
  <li class="breadcrumb-item">Tables</li>
  <li class="breadcrumb-item active"><a href="#">Data Order</a></li>
</ul>
</div>
...
<div class="tile-body">
  ...
  <form class="row" method="POST" action="">
    <div class="form-group col-md-3">
      <input class="form-control" type="" placeholder="Select Start Date" id="startDate" name="startDate" autocomplete="off" value="<?= $startDate ?>">
    </div>
    <div class="form-group col-md-3">
      <input class="form-control" type="" placeholder="Select End Date" id="endDate" name="endDate" autocomplete="off" value="<?= $endDate ?>">
    </div>
			...
			<table class="table table-bordered table-striped table-condensed" id="" style="width: 600px; margin:0 auto">
				...
				<tbody>
					<?php 
						...
						$tKiloan = 0;
						$tPotongan =0;
						$sql = $con-> query("SELECT * FROM reception a, outlet b WHERE a.nama_outlet=b.nama_outlet AND a.cara_bayar<>'Void' AND a.cara_bayar<>'Reject' AND a.rijeck=false AND b.Kota='$cabang' AND a.nama_outlet='$outlet' AND DATE_FORMAT(a.tgl_input, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate' GROUP BY a.nama_outlet ASC ");
  ...
  <div class="table-responsive">
  	<table class="table table-hover table-striped" id="omsetTable">
        ...
        <tbody>
          ...
          $sql = $con-> query("SELECT * FROM reception a, outlet b WHERE a.nama_outlet=b.nama_outlet AND a.cara_bayar<>'Void' AND a.cara_bayar<>'Reject' AND b.Kota='$cabang' AND a.nama_outlet='$outlet' AND a.rijeck=false AND DATE_FORMAT(a.tgl_input, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'");
          while($data = $sql-> fetch_array()){
          	...
          	<tr>
          	  ...
          	  <td><?= $data['no_nota'] ?></td>	
          	  <td id="<?= encrypt($data['id_customer'],$key)  ?>" class="riwayat-order"><a href="#" style="color:black"><?= $data['nama_customer'] ?></a></td>	

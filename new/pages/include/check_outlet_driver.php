...
<script>
    jQuery(function ($) {
        ...
        let checkType = check == "kotor" ? "out" : "in";
        let textHeader = check == "kotor" ? "Checkout Outlet Cucian Oleh "+deliveryName : "Checkin Outlet Cucian Oleh "+deliveryName;
        let sendId = check == "kotor" ? outlet : deliveryId;
        ...
        getData("LaundryTracker/getToCheckOutlet/"+sendId, { outlet: outlet, checkType: checkType }, function (response) {
            if (response.readyState == 0) {
                ...
                $("#sales_order").on("keypress", function (e) {
                    ...
                    error.message = "Nota tidak ditemukan, periksa status posisi terakhir nota";
                            ...
                            if(value.sales_order == salesOrder) {
                                ...
                                if (checkType == "in") {
                                    if (value.outlet != outlet) {
                                        cek = false;
                                        error.message = "Nota bukan milik outlet "+outlet
                                    } else {
                                        cek = true;   
                                        listOrder.push({
        ...
        function submitForm(data) {
            getData("CheckOutletDelivery/store", data, function (response) {
                if(response.readyState == 0) {
                    ...
                    // $("#panel_driver .panel-body").html($('<div class="input-group"><input type="text" class="form-control" id="phone"><span class="input-group-btn"><button class="btn btn-sm btn-success" type="button" id="send_whatsapp"><i class="fa fa-whatsapp" aria-hidden="true"></i></button></span></div>'));
        ...
        function signOutDriver(driverId, checkType) {
            ...
            getData("Driver/signOut/"+driverId, { }, function (response) {
                if (response.readyState == 0) {
                    ...
                    if (checkType == "in") {
                        window.location.href = "?menu=checkin_outlet";
                    } else {
                        window.location.href = "?menu=checkout_outlet";
                    }
                }
            });
        }

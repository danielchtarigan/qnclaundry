		...
		<div class="widget-main table-responsive">
			<table class="table table-bordered table-condensed" id="antrian">
				...
				<tbody>
					...
					{
					    $query = mysqli_query($con, "SELECT * FROM reception WHERE workshop='$outlet' AND jenis='l' AND spk=true AND packing=false AND kembali=false AND cara_bayar<>'Void' AND cara_bayar<>'Reject' AND (cuci=false OR pengering=false) AND rijeck=false ORDER BY tgl_input ASC");
					}
					
					else
					{
					    $query = mysqli_query($con, "SELECT * FROM reception WHERE (nama_outlet='$outlet' OR workshop='$outlet') AND jenis='l' AND spk=true AND packing=false AND kembali=false AND cara_bayar<>'Void' AND cara_bayar<>'Reject' AND (cuci=false OR pengering=false) AND rijeck=false ORDER BY tgl_input ASC");
					}
		...
		<div class="widget-main table-responsive">
			<table class="table table-bordered table-condensed" id="antrian2">
				...
				<tbody>
					...
					if($_SESSION['level']=="mitra")
					{
					    $query = mysqli_query($con, "SELECT * FROM reception WHERE workshop='$outlet' AND jenis='l' AND spk=true AND packing=false AND kembali=false AND cuci=true AND pengering=true AND (setrika=false OR packing=false) ORDER BY tgl_input ASC");
					}
					else
					{
					   $query = mysqli_query($con, "SELECT * FROM reception WHERE (nama_outlet='$outlet' OR workshop='$outlet') AND jenis='l' AND spk=true AND packing=false AND kembali=false AND cuci=true AND pengering=true AND (setrika=false OR packing=false) ORDER BY tgl_input ASC"); 
					}

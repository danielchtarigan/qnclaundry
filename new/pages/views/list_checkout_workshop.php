...
<script type="text/javascript">
	$(function(){
		...
		$('button#cari').on('click', function (e) {
			...
			datatable(startDate, endDate, outletId, "out");
		});
        ...
        datatable(nowDate, nowDate, outletId, "out");
		...
		function datatable(startDate, endDate, outletId, type) {
			...
			let datatable = $('#omset').DataTable({
				"processing": true,
				"ajax": {
					url: apiURL+"CheckWorkshopDelivery/list",
					type: "POST",
					data: {startDate:startDate, endDate:endDate, workshop_id:outletId, type: type},
				...
				"columns": [
					...
					{ "data": "code", render: function (data, type, row) {
						return '<a href=\"document/checkout.php?wk&d='+data+'\" target=\"_blank\">'+data+'</a>';
					}},
					{ "data": "driver" },
					{ "data": "kasir" },
					{ "data": "count" }

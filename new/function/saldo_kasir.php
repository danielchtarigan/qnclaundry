<?php 
   ...
   function all_cash_orders($nama,$tanggal_mulai,$tanggal_akhir){
    ...
    function hutang_customer($nama,$tanggal_mulai){
       global $con;
        $sql = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS total FROM reception WHERE lunas=false AND nama_reception='$nama' AND DATE_FORMAT(tgl_input, '%Y-%m-%d') >= '$tanggal_mulai'");
        $data = mysqli_fetch_row($sql);
        return $data[0];
    }
    ...
    function lap_sisa_saldo($nama,$tanggal_mulai,$endDate){
        ...
        $data['hutang'] = hutang_customer($nama,$tanggal_mulai);
    ...
    function laporan_saldo($nama,$tgl,$tanggal_mulai,$tanggal_akhir){
        $data['debet'] = cash_order($nama,$tgl)+cash_langganan($nama,$tgl)-pengeluaran($nama,$tgl)+setoran_delivery($nama,$tgl);

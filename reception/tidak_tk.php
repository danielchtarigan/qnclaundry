...
<th>Outlet</th>
		...
		<tbody>
			<?php
			$query = "SELECT a.nama_outlet,DATE_FORMAT(a.tgl_input, '%Y-%m-%d') as tgl_input,sum(a.total_bayar) as jumlah,a.nama_reception FROM reception a left join tutup_kasir b ON (a.nama_reception=b.reception and DATE_FORMAT(a.tgl_input, '%Y-%m-%d')= DATE_FORMAT(b.tanggal, '%Y-%m-%d'))  where a.cara_bayar='cash' and b.reception is null and (DATE_FORMAT(a.tgl_input, '%Y-%m-%d') between '$newDate' and '$newDate2')  group by a.nama_reception,a.nama_outlet,DATE_FORMAT(a.tgl_input, '%Y%m%d') ORDER BY a.tgl_input ASC" ;

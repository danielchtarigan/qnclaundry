...
if($_GET['batal']=='undefined'){
    ...
    if(mysqli_num_rows($cekerror)>0){ ?>
        <script type="text/javascript">
            ...
            location.href="index.php?menu=pembatalan";
        </script>
    <?php
    } else{
        ...
        if($_GET['batal']=="void"){
            ...
            $updateorder = mysqli_query($con, "UPDATE reception SET cara_bayar='Reject', spk='1' WHERE no_nota='$_GET[order]'");
        }
        ...
        mysqli_query($con, "INSERT INTO order_batal (tanggal,rcp_input,tgl_order,rcp_order,no_order,harga,no_faktur,alasan,status) VALUES ('$nowDate','$rcpInput','$row[tgl_input]','$row[nama_reception]','$_GET[order]','$row[total_bayar]','$row[no_faktur]','$_GET[als]','$_GET[batal]')");
         ...
         //Mengembalikan Saldo Subagen (Jika order berasal dari subagen)

            $subAgen = $row['nama_reception'];

            $qsaldoSa = $con -> query("SELECT * FROM saldo_subagen WHERE subagen='$subAgen'");
            $rSaldoSa = $qsaldoSa -> fetch_array();
            $saldoTerakhir = $rSaldoSa['bonus'];

            $saldoSekarang = $saldoTerakhir+$row['total_bayar'];

            $con -> query("UPDATE saldo_subagen SET bonus='$saldoSekarang' WHERE subagen='$subAgen'");

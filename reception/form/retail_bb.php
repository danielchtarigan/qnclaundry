<?php
$kode_bin="";
$qbin = mysqli_query($con, "select * from bin_bni");
$jb= mysqli_num_rows($qbin);
$it= 1;
while ($rbin = mysqli_fetch_array($qbin)){  
$kode_bin.="'".$rbin['nomor_bin']."'";
$it++;
if ($it<=$jb) {
$kode_bin.=",";
}
...
function validasi()
    {
        var nip=document.forms["form3"]["cardcode"].value;
        var cc=document.getElementById("cardcode");
        var numbers=/^[0-9]+$/;
        var str=document.form3.cardcode.value;
        var str1 = str.substr(0,6);
        var kode = [<?=$kode_bin?>];
        var position  = kode.indexOf(str1);  
        if (cc.readOnly==false){
        if (nip==null || nip=="")
          {
          alert("Kode Bin tidak boleh kosong !");
          return false;
          };
        if (!nip.match(numbers))
          {
          alert("Kode Bin harus angka !");
          return false;
          };
        if (nip.length!=16)
          {
          alert("Kode Bin harus 16 digit !");
          return false;
          };
         if (position<0)
          {
          alert("Verifikasi Kode Bin Gagal / Periksa Kode Bin !!");
          return false;
          };
        };
     }
...
function edc(){
  //if (g=="BNI"){
var answer=document.getElementById("bankedc");
 if(answer[answer.selectedIndex].value=="BNI"){
 document.form3.nilaiedc.readOnly = false;
 document.form3.cardcode.readOnly = false;
}
else if (answer[answer.selectedIndex].value!=""){
 document.form3.nilaiedc.readOnly = false;
 document.form3.cardcode.readOnly = true;
  document.form3.cardcode.value = '';
}
else{
 document.form3.nilaiedc.readOnly = true;
 document.form3.cardcode.readOnly = true;
 document.form3.nilaiedc.value = '';
 document.form3.cardcode.value = '';
}
}
...
<form name="form3" id="form3" action="act_penjualan.php" method="GET" class="form-horizontal" onSubmit="return validasi()"><br>
...
<?php
 ...
 <tr>
  ...
  <td align="right">
   ...
   <input type="text" placeholder="Kode Bin BNI" id="cardcode" name="cardcode" class="form-control" style="width:100%" value="" readonly="readonly" >
  </td>
 </tr>
 <tr>

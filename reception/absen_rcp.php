...
<td><?php
	$sql4 = mysqli_query($con,"SELECT reception,DATE_FORMAT(tanggal, '%Y-%m-%d') as tanggal,TIME_FORMAT (tanggal , '%H:%i:%s') as pulang, outlet FROM tutup_kasir WHERE DATE_FORMAT(tanggal,'%Y-%m-%d')='$tgl' and reception='$data[id_user]' and outlet='$data[id_outlet]'" );
	
	$s2=mysqli_fetch_array($sql4); echo $s2['pulang'] ?></td>
	<td><?php
		$time1 = $data['masuk'];
		$time2 = $s2['pulang'];
		$sql5 = mysqli_query ($con,"SELECT TIMEDIFF ('$time2', '$time1') as durasi");
		$s3 = mysqli_fetch_array ($sql5); echo $s3['durasi']
		?></td>
					</tr>

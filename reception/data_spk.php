...
<fieldset>
<legend align="center"><strong>Jumlah SPK Harian Resepsionis</strong></legend>
dari tanggal <?php echo $newDate ;?> sampai <?php echo $newDate2; ?> 
	<table id="cuci" class="display">
		...
		<tbody>
			<?php
			$query = "SELECT nama_outlet,DATE_FORMAT(tgl_spk, '%Y/%m/%d') as tgl_transaksi,rcp_spk,sum(total_bayar) as jumlah,lunas FROM reception  WHERE (DATE_FORMAT(tgl_spk, '%Y-%m-%d') between '$newDate' and '$newDate2')  group by rcp_spk,DATE_FORMAT(tgl_spk, '%Y%m%d'),nama_outlet,lunas ORDER BY tgl_input ASC" ;
			$tampil = mysqli_query($con, $query);
			
			while($data = mysqli_fetch_array($tampil)){
				$kom=$data['jumlah']*4/100
...
<legend align="center"><strong>Rincian SPK</strong></legend> 
	<table id="rinciancash" class="display">
		...
		<tbody>
			<?php
			$query = "SELECT tgl_input,no_nota,total_bayar,tgl_spk,rcp_spk,lunas,nama_outlet,nama_customer FROM reception  WHERE (DATE_FORMAT(tgl_spk, '%Y-%m-%d') between '$newDate' and '$newDate2') " ;
			$tampil = mysqli_query($con, $query);

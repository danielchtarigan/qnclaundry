	...
	switch($jenis){
		case "ceknofaktur":
			...
			if($stmt->fetch()){
				...
				$stmt->close();
				$stmt_reception = $con->prepare("SELECT c.nama_customer, no_telp, alamat FROM reception r INNER JOIN customer c ON r.id_customer=c.id WHERE (no_nota=? OR no_so=?) AND lunas = 0 AND cabang != ''");
				$stmt_reception->bind_param("ss",$nofaktur,$nofaktur);
				$stmt_reception->execute();
				$stmt_reception->bind_result($data['nama_customer'],$data['no_telp'],$data['alamat']);
				if($stmt_reception->fetch()){
					$json['status'] = "yes";
					$json['nama'] = $data['nama_customer'];
					$json['nohp'] = $data['no_telp'];
					$json['alamat'] = $data['alamat'];
			...
			if(!empty($_POST['g-recaptcha-response'])){
				...
				if($response_json->success){
					...
					if($stmt->fetch()){
						...
						$stmt->close();
						$stmt_reception = $con->prepare("SELECT id_customer FROM reception WHERE (no_nota=? OR no_so=?)");
						$stmt_reception->bind_param("ss",$no_faktur,$no_faktur);
						$stmt_reception->execute();
						$stmt_reception->bind_result($data['id_customer']);
						if($stmt_reception->fetch()){
							$stmt_reception->close();
							$customer_exist = true;
						} else {
					...
					if($customer_exist) {
						...
						if($stmt->execute()){
							...
							Redirect('http://qnclaundry.com/reception/data_delivery.php');
						} else {

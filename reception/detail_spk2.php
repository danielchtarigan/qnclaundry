...
<?php
          ...
          $allowed = true;
          if ($r['express']==0 && $r['priority']==0) {
              $ra = mysqli_query($con,"SELECT tgl_input,no_nota,nama_customer,express,priority,lunas FROM reception where nama_outlet='$ot' and ambil=false and kembali=false and spk=false and lunas=true ORDER BY tgl_input ASC");
              $rq = mysqli_fetch_array($ra);
              $rm =$rq['tgl_input'];
              $rw = strtotime($rm);
              $rnow = strtotime($now);
              $selisih = abs($rnow-$rw);
              
              if ($rm<$sq && $selisih >= 18000) {
                $allowed = false;
              }
              else if ($rm<$sq && $selisih < 18000){
                $allowed = true;
              }
              else{
                $allowed = true;
          ...
          if ($allowed) { ?>
             <button id="selesaiSpk" class="btn btn-md btn-danger">Selesai</button>
            ...
            ?>
              ...
              <span id="status"></span>
          <?php } else {            
            echo '<p style="color:red"><b>ADA CUCIAN SUDAH LEBIH DARI 5 JAM BELUM DISPK, SPK LEBIH DULU:</b><br>';            
              echo "$rq[tgl_input] - $rq[no_nota] - $rq[nama_customer]<br><br>";
              echo 'Nota '.$rq['no_nota'].' Void???kirim di <a href="index.php?menu=void"><strong>Form Void</a></strong><br>Konfirmasi via SMS ke 081233323008 atau BBM D6ABEEE4';
              echo '</p>';
          } ?>

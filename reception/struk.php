...
<div class="content" id="content">
    <em style="font-size: 8px">Struk Customer</em>
    <div style="max-width:80mm;margin: 1mm 3mm;">
        <?php include"bar128.php" ?>
        <div align="center"><img src="../logo.bmp" />
        </div>   
...
//menghapus data dari tabel tmp         
          $qtmp = mysqli_query($con, "DELETE from order_potongan_tmp WHERE id_customer='$idc' AND cabang<>'Delivery'");
          $qtmp = mysqli_query($con, "DELETE from order_tmp where id_customer='$idc' AND cabang<>'Delivery'");  
         ...
         <div align="center"><?php echo bar128(stripslashes($new_nota)); ?><br>
            <?php if ($rrec['priority']==1) { ?><div style="font-size:12pt;font-family: Arial Black;border:2px solid black">PRIORITY<br>CUSTOMER</div><br><?php } ?>
                <?php echo $no_nota; ?></div>
                <br>
...
<?php 
$cbOrder = mysqli_query($con, "SELECT * FROM voucher_cashback_order WHERE status=false AND id_customer='$idc' AND no_nota='$no_nota' ORDER BY id DESC");

if(mysqli_num_rows($cbOrder)>0){
    ?>
    <div style="border-top: 3px dashed">
        <?php 
        //Start tampilkan voucher_cashback_order
        $selamat = "SELAMAT! ANDA MENDAPATKAN";      
        $outlett = "SEMUA OUTLET QNCLAUNDRY";  
        
        while($datacbOrder = mysqli_fetch_assoc($cbOrder)){
          $kode = $datacbOrder['kode_voucher'];
          $voucher = "CASHBACK ORDER Rp. ".number_format($datacbOrder['nominal'],0,',','.'); ?>
          <table style="width:100%;border-bottom: 1px dashed #000;font-size: 7pt;font-family: Tahoma; margin-top: 5px;">
            <tr>  
                 <th align="center"><?php
                  echo $selamat.'<br>'.$voucher.'<p style="font-size:9px; color:green;">Berlaku Senin s/d Kamis dan '.date('d/m/Y', strtotime($datacbOrder['mulai'])).' hingga '.date('d/m/Y', strtotime($datacbOrder['akhir'])).bar128($kode).'</p> Voucher berlaku untuk satu order<br><br>'.'TANPA MINIMAL TRANSAKSI<br>';
                  ?> 
                </th> 
              </tr>
          </table>
      ...
      //End tampilkan voucher_cashback_order
        ?>
    </div>
    <br><br><br>
<?php
...
<div style="page-break-before:always;">
    ...
    <div style="font-size: 9pt; font-family: Tahoma" >
     ...
     <div align="center" style="margin-top:5px;"><?php echo bar128(stripslashes($no_nota))?><br>
        <?php if ($rrec['priority']==1) { ?><div style="font-size:12pt;font-family: Arial Black;border:2px solid black">PRIORITY<br>CUSTOMER</div><br><?php } ?>
        </div>
           <table style="border-top: 1px dotted #000;width:100%;">
               <tr>

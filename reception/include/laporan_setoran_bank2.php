					...
					$rcp = $reception;
						...
						for ($no; $no>9; $no--){
							...
							$setoran2 = 0;
							$sesuai2 = 0;
							   $q2 = mysqli_query($con, "select sum(jumlah) as total from cara_bayar where resepsionis='$rcp' and tanggal_input like '%$tanggal%' and cara_bayar='Cash'");
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example" style="font-size:12px;">
                                    ...
                                    <?php
										...
										for($no; $no>9; $no--){
											$tanggal = $tahun.'-'.$bulan.'-'.sprintf('%02s', $no);
													$cekrcp = mysqli_query($con, "select * from reception where tgl_input like '%$tanggal%' and rcp_lunas='$_SESSION[user_id]' group by rcp_lunas");
...
$ot = $rcekrcp['nama_outlet'];
									?>
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo $ot; ?></td>
                                            <?php
                                            $cash = mysqli_query($con, "select sum(jumlah) as cas from cara_bayar where resepsionis='$rcp' and tanggal_input like '%$tanggal%' and cara_bayar='Cash'");

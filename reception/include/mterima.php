...
<h3>MANIFEST TERIMA OUTLET</h3><?php
if (isset($_POST['nota']) and isset($_POST['jumlah']) and isset($_POST['driver'])) { //form simpan data manifest terima
...
<table width="100%">
  ...
  $i=0;
  	foreach($kode_serah as $key => $value){
  		if ($value!=''){
  	$cekdata=mysqli_query($con,"SELECT * FROM manifest WHERE kd_serah3='$value'");
  	echo '<h4>'.$value.'</h4>';
...
//=================================================
else if (isset($_POST['driver'])) {//form pilih data manifest serah
	$driver=$_POST['driver'];
?>
<p><input type="text" class="form-control" name="cari" id="cari" onkeypress="handle(event)" placeholder="Scan Kode Manifest Serah di sini !" style="float:left;"></p>
<table width="100%" >
  <tr><td width="50%">
    <div id="checkboxes">
  <?php
  $cekdata = mysqli_query($con, "select * from man_serah where driver='$driver' and tipe=3 and kode_terima='' and tempat='$ot'");
  $ncek = mysqli_num_rows($cekdata);
  if ($ncek>0){
    $i=0;
    while ($rcekdata = mysqli_fetch_array($cekdata)){
...
<form method="POST" action="">
    <div>                                       
        <input type="text" name="driver" class="form-control" placeholder="Masukkan Nama Driver Disini.." style="width:50%; float:left;"/>      											
        <br><br>

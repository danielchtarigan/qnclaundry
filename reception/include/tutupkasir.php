...
<div align="center">
	...
	<div class="easyui-panel" title="Tutup Kasir" align="center" style="width:500px;padding:10px;">
		...
		$uang = mysqli_query($con, "select sum(b.jumlah) as cash from reception a, cara_bayar b where a.tgl_input like '%$tgl%' and a.rcp_lunas='$_SESSION[user_id]' and a.nama_outlet='$_SESSION[nama_outlet]' and a.no_faktur=b.no_faktur and b.cara_bayar='Cash'");
		$ruang = mysqli_fetch_array($uang);
		$cash = $ruang['cash']+$uangbuka;
		?>
  		 <div class="col-xs-8">
  		<input type="number" autocomplete="off" class="form-control" name="setoran_bersih" id="setoran_bersih" value="0" required="true" />
  		</div>
		</div>
<?php
...
?>
		<div class="form-group">
  		<label for="no_nota" class="control-label col-xs-3">EDC MANDIRI</label>
  		 <div class="col-xs-8">
  		<input type="number" autocomplete="off" class="form-control" name="edc_mandiri" id="edc_mandiri"  value="0" required />
  		</div>
		</div>

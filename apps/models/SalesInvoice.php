...
class SalesInvoice {
    ...
    public function getOmsetByOutlet($data)
    {
        ...
        $this->conn->query("SELECT SUM(IF(jenis_transaksi = 'ritel' AND b.cara_bayar <> 'Kuota', total, 0)) AS laundry,
                        SUM(IF(jenis_transaksi = 'ritel' AND b.cara_bayar = 'Kuota', total, 0)) AS kuota,
                        ...
                        (SUM(total) - SUM(IF(jenis_transaksi = 'ritel' AND b.cara_bayar = 'Kuota', total, 0))) AS total,
                        DATE(tgl_transaksi) AS tgl, rcp, a.nama_outlet FROM $this->table AS a LEFT JOIN $paymentMethod AS b ON a.no_faktur = b.no_faktur
                        WHERE a.nama_outlet = :outlet

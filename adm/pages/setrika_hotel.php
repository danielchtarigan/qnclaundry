...
<body>
	...
	<table id="packing" class="display">
		...
		<tfoot>
            <tr>
                ...
                <th></th>
						...
						<tbody>
				<?php
				$user_query = $mysqli->query("SELECT DATE_FORMAT(tgl_setrika, '%Y/%m/%d') as tgl_setrika,user_setrika,sum(berat) as beratsetrika FROM setrika_hotel GROUP BY DATE_FORMAT(tgl_setrika, '%Y/%m/%d'), user_setrika ORDER BY tgl_setrika DESC" )or die(mysqli_error());
		...
		<tbody>
			<?php
			$query =$mysqli->query("SELECT * FROM setrika_hotel ORDER BY tgl_setrika DESC") ;

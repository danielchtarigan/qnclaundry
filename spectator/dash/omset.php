...
function hari($date){
	global $con;
	$query = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS omset FROM reception WHERE tgl_input LIKE '$date%' AND nama_outlet='$_SESSION[outlet]' AND cara_bayar<>'Void'");
...
function hari_kiloan($date){
	global $con;
	$query = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS omset FROM reception WHERE tgl_input LIKE '$date%' AND nama_outlet='$_SESSION[outlet]' AND cara_bayar<>'Void' AND jenis='k'");
	$data = mysqli_fetch_array($query);
	$omset = $data['omset'];
	return $omset;
}
...
function hari_potongan($date){
	global $con;
	$query = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS omset FROM reception WHERE tgl_input LIKE '$date%' AND nama_outlet='$_SESSION[outlet]' AND cara_bayar<>'Void' AND jenis='p'");
	$data = mysqli_fetch_array($query);
	$omset = $data['omset'];
	return $omset;
}

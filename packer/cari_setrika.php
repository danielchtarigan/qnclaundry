...
if ( $row != 0 ) {
    ...
    $.ajax({
        ...
        type     : 'POST',
        dataType : 'html',
        data     : 'no_nota='+no_nota+'&jumlah1='+jumlah1+'&ket='+ket+'&setrika='+setrika+'&berat='+berat+'&id_cs='+id_cs,
...
printout();
    ...
    newWindow.print();
		...
		<div id="pesan_kirim" style="display:none"></div>
  		...
  		<input type="text" class="form-control" name="id_cs" id="id_cs" required="true" value="<?php echo $data['id_customer']; ?>" readonly="true" >
	...
	<select class="form-control" name="setrika" id="setrika" required=true>
        <option value="">--</option>';
			...
			$query = "SELECT * from user where (level='setrika' OR izinkan='setrika') AND aktif='Ya'";
    ...
    if ($data['express']==0 && $data['priority']==0) {
        $sqlbelumdiproses = mysqli_query($con,"SELECT no_nota,tgl_input FROM reception WHERE ws_setrika='$ws' and (tgl_input < '".$data['tgl_input']."') and nama_outlet<>'mojokerto' and cuci=true and ((jenis='k' and pengering=true) or (jenis='p')) and setrika=false and spk=true and packing=false and kembali=false and ambil=false and jenis='".$data['jenis']."' and rijeck=false ORDER BY tgl_input ASC");
    ...
    <?php } else {
      ...
      echo '<p style="color:red"><b>WAJIB MENYETRIKA ANTRIAN '.$type.' DARI TANGGAL LEBIH TUA TERLEBIH DAHULU YAITU:</b><br>';
      while ($data = mysqli_fetch_array($sqlbelumdiproses))
      {
        echo $data['tgl_input'].' - '.$data['no_nota'].'<br>';
      }
      echo '</p>';
    } ?>

...
if($jenis=='A') {
	function frek_trx($startDate,$endDate,$idcs) {
		...
		$sql = mysqli_query($con, "SELECT COUNT(DISTINCT DATE_FORMAT(tgl_input, '%Y/%m/%d')) AS jum FROM reception WHERE id_customer='$idcs' AND (DATE_FORMAT(tgl_input, '%Y/%m/%d') BETWEEN '$startDate' AND '$endDate')");
	...
	function total_trx($startDate,$endDate,$idcs) {
		...
		$sql = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS total FROM reception WHERE id_customer='$idcs' AND (DATE_FORMAT(tgl_input, '%Y/%m/%d') BETWEEN '$startDate' AND '$endDate')");
		$data = mysqli_fetch_row($sql);
		return $data[0];
	}
}
	...
	function frek_trx($startDate,$endDate,$idcs) {
		global $con,$frek,$jenis;
		$sql = mysqli_query($con, "SELECT COUNT(DISTINCT DATE_FORMAT(tgl_input, '%Y/%m/%d')) AS jum FROM reception WHERE id_customer='$idcs' AND (DATE_FORMAT(tgl_input, '%Y/%m/%d') BETWEEN '$startDate' AND '$endDate') AND jenis='$jenis'");
		$data = mysqli_fetch_row($sql);
		return $data[0];
	}
	...
	function total_trx($startDate,$endDate,$idcs) {
		global $con,$min_tr,$max_tr,$jenis;
		$sql = mysqli_query($con, "SELECT COALESCE(SUM(total_bayar),0) AS total FROM reception WHERE id_customer='$idcs' AND (DATE_FORMAT(tgl_input, '%Y/%m/%d') BETWEEN '$startDate' AND '$endDate') AND jenis='$jenis'");
		$data = mysqli_fetch_row($sql);
		return $data[0];
	}
}

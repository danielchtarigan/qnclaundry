...
include "../config.php"; 
$op=$_SESSION['user_id'];
$sql="SELECT vl.*,c.nama_customer nama,ve.created,ve.expired,ve.user,ve.outlet FROM voucher_lucky vl left join customer c on vl.id_customer = c.id "
        . " left join voucher_expired ve on vl.no_voucher = ve.voucher"
        . " where vl.aktif=0 and id_customer<>false and vl.kali<=10 and vl.jenis_voucher='RV';";
?>
<script>
...
<div class="container" style="padding:20px; margin:100px auto; position:relative; border:3px solid rgba(0,0,0,0); -webkit-border-radius:5px; -moz-border-radius:5px; border-radius:5px; -webkit-box-shadow:0 0 18px rgba(0,0,0,0.4); -moz-box-shadow:0 0 18px rgba(0,0,0,0.4); box-shadow:0 0 18px rgba(0,0,0,0.4);  margin-bottom:20px; color:#000000;">
        <center><h2>Laporan Pemilik Voucher Referral</h2></center>
			...
			<th>Keterangan</th>                        
				...
				while($r = mysqli_fetch_array($user_query)){
					...
					$jenis="Diskon";
                                ...
                                ?>
                             <tr>
                                    <td><?= $r['created']!="" ? date('d M Y',  strtotime($r['created'])) : ""; ?></td>
                                    ...
                                    <td><?= $r['user']; ?></td>
                                    <td><?= $r['outlet']; ?></td>
                                    <td><?= $r['expired']!="" ? date('d M Y',  strtotime($r['expired'])) : ""; ?></td>

                                ...
                                <table class="table table-striped table-bordered table-hover" id="datasetoran" style="font-size:9px;">
                                    ...
                                    <?php										
												...
												while ($rq1 = mysqli_fetch_array($q1)){
													$cekrcp = mysqli_query($con, "select DATE_FORMAT(tgl_input, '%Y-%m-%d') as tanggal,rcp_lunas,nama_outlet from reception where (DATE_FORMAT(tgl_input, '%Y-%m-%d') between '$newDate' and '$newDate2') and nama_outlet='$rq1[nama_outlet]' group by DATE_FORMAT(tgl_input, '%Y-%m-%d'),rcp_lunas order by DATE_FORMAT(tgl_input, '%Y-%m-%d') DESC");

	...
	<div class="table-responsive" style="margin-top: 25px">
		...
		<table class="table table-striped" style="font-size: 11px" id="datatdp" width="100%">
			<thead>
				<tr>
					<th>No</th>
					<th>Tanggal Input</th>
			...
			<tbody>
				<?php 
				$i = 1;
				$sql = $con-> query("SELECT tgl_input, spk, no_nota, jenis,nama_customer,total_bayar,tgl_cuci,tgl_setrika,tgl_packing,timediff('$jam',tgl_input) as waktu,tgl_workshop,op_workshop, op_cuci, user_setrika, user_packing, tgl_spk, rcp_spk FROM reception WHERE jenis='k' AND nama_outlet<>'mojokerto' AND workshop='Toddopuli' AND kembali=false AND ambil=false AND tgl_so='0000-00-00 00:00:00' AND rijeck=false AND packing=false AND cara_bayar<>'Void' AND cara_bayar<>'Reject' AND status_order='' ORDER by tgl_input ASC");
	...
	<div class="table-responsive" style="margin-top: 25px; border-top: 2px solid black">
		...
		<table class="table table-striped" style="font-size: 11px" id="datatdppotongan" width="100%">
			<thead>
				<tr>
					<th>No</th>
					<th>Tanggal Input</th>
			...
			<tbody>
				<?php 
				$i = 1;
				$sql = $con-> query("SELECT tgl_input, spk, no_nota, jenis,nama_customer,total_bayar,tgl_cuci,tgl_setrika,tgl_packing,timediff('$jam',tgl_input) as waktu, tgl_workshop,op_workshop, op_cuci, user_setrika, user_packing, tgl_spk, rcp_spk FROM reception WHERE jenis='p' AND nama_outlet<>'mojokerto' AND workshop='Toddopuli' AND kembali=false AND ambil=false AND tgl_so='0000-00-00 00:00:00' AND rijeck=false AND packing=false AND cara_bayar<>'Void' AND cara_bayar<>'Reject' AND status_order='' ORDER by tgl_input ASC");
				while($data = $sql-> fetch_array()){
					$waktus = explode(":",$data['waktu']); 

...
<fieldset>
<legend align="center" >DAFTAR PENCAPAIAN POIN DAN KPI OPERASIONAL</legend>
	...
	<tr>
		...
		$packingexp = mysqli_query($con, "SELECT COUNT(*) as bonus, user_packing FROM reception WHERE user_packing='$datauser[name]' AND (express='1' OR express='2' OR express='3')  AND (DATE_FORMAT(tgl_packing, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate') ");
			...
			if($datauser['level']=='setrika' && $datauser['jenis']=='potongan'){
				...
				$bonusop2 = ($omsetpotongan/$datakerjaopr[0]*$kerjareg)*0.02/800/$jumSetrika;
			}else if($datauser['level']=='operator' && $datauser['jenis']=='potongan'){
				$ketbonus2 = "Omset Potongan 4%";
				$bonusop2 = ($omsetpotongan/$datakerjastr[0]*$kerjareg)*0.04/1100/$jumOperator;			
			}else{
				$bonusop2 = 0;
				$ketbonus2 = "--";
			}
			...
			if($datauser['level']=='operator'){
			    if($datauser['jenis']=='potongan'){
			        $pencapaian = ($pbrosur+$cucikiloan+$keringkiloan+$psetrika+$cucipotongan+$poinhotel+$packingkiloan+$packingpotongan+$insentifmalam+$bonusop2-$denda);
				...
				$kekuranganpoinperbulan = ($rataharian-$target)*$kerjareg;	
				if($datauser['jenis']=='potongan'){					
					$pencapaian2 = ($pbrosur+$cucikiloan+$keringkiloan+$cucipotongan+$poinhotel+$packingkiloan+$packingpotongan+$insentifmalam+$bonusop2-$denda);
					$poinmin2 = 0;
					$poinnonbonus2 = 0;
